...
class Watcher(object):
    ...
    def __init__(self, file_path, test_program, debug=False, custom_args='',
                 ignore_dirs=None, quiet=False, manage_py_path=''):
        # Safe filter
    ...
    def check_configuration(self, file_path, test_program, custom_args):
        ...
        if not test_program in IMPLEMENTED_TEST_PROGRAMS:
            raise InvalidTestProgram(
                'The `%s` is unknown, or not yet implemented. Please chose another one.' % test_program)

        if custom_args:
            if not self.quiet and not ask(
    ...
    def get_cmd(self):
        ...
        if self.test_program in ('nose', 'nosetests'):
            ...
            if self.manage_py_path:
                cmd = 'python %s/manage.py test' % self.manage_py_path
            else:
        ...
        if not cmd:
            raise InvalidTestProgram(
                "The test program %s is unknown. Valid options are: `nose`, `django` and `py`" % self.test_program)
...
def main(prog_args=None):
    ...
    parser.add_option(
        ...
        default='',
        type="str",
        help="Defines the path to the manage.py file for django tets.")
    ...
    try:
        watcher = Watcher(path, opt.test_program, opt.debug, opt.custom_args,
                          opt.ignore_dirs, opt.quiet, opt.manage_py_path)
        ...
        if watcher_file_size > opt.size_max:
            ...
            watcher_file_size, opt.size_max)
